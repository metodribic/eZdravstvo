...
CREATE TABLE `users` (
  ...
  `sifra` varchar(4) DEFAULT NULL,
  `role` int(11) DEFAULT NULL,
  `ambulanta` int(11) DEFAULT NULL,
  `email` varchar(45) NOT NULL,
  `password` varchar(45) NOT NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `email_UNIQUE` (`email`),
  KEY `fk_users_1_idx` (`role`),
  KEY `fk_users_2_idx` (`ambulanta`),
  CONSTRAINT `fk_users_1` FOREIGN KEY (`role`) REFERENCES `roles` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  CONSTRAINT `fk_users_2` FOREIGN KEY (`ambulanta`) REFERENCES `ambulanta` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION
